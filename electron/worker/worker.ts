...
// 校对机位置触发
const ProofreadingPositionTrigger = func({
  assemblyFile: url,
  typeName: "Measurement.Entrance",
  methodName: "ProofreadingPosition",
})

// 启动试调
const StartProofreading = func({
  assemblyFile: url,
  typeName: "Measurement.Entrance",
...
// 写入补偿值
function WriteStandardProductExecution(data: string) {
  let b: boolean;
...
on("worker-receive-verify-start", (event: any) => {
  ...
  // event.sender.send('worker-send-verify-judgment', true)
})

// 工作进程对机前执行
on("worker-receive-compensate-execute", (event: any) => {
  if (WriteStandardProductExecution('0_0_0_0')) {
...
// 校对机位置触发
on("worker-receive-proofreading-position-trigger", (event, index: number) => {
  ProofreadingPositionTrigger(index, (error: any, result: any) => {
    if (error) {
      event.sender.send('worker-send-dll-error', '校对机位置触发失败');
      return
    }
    if (result == true) {
      event.sender.send('worker-send-dll-success', '校对机位置触发成功');
    } else {
      event.sender.send('worker-send-dll-error', '校对机位置触发失败');
    }
  })
...
// 试调循环
on("worker-receive-proofreading-loop", (event) => {
  event.sender.send('worker-send-proofreading-loop', MeasureStart())
  // event.sender.send('worker-send-proofreading-loop', true)
})
